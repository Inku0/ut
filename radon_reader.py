#!/usr/bin/python3
# credit to https://github.com/EtoTen/radonreader/
...
from radon_reader_by_handle import radon_device_finder, radon_device_reader, radonDataRAW, ScanDelegate, ReadDelegate, \
    nConnect
...
parser.add_argument('-b', '--becquerel', action='store_true', help='Display radon value in Becquerel (Bq/m^3) unit',
                    required=False)
parser.add_argument('-v', '--verbose', action='store_true', help='Verbose mode', required=False)
parser.add_argument('-s', '--silent', action='store_true', help='Output only radon value (without unit and timestamp)',
                    required=False)
...
parser.add_argument('-mp', dest='mqtt_port', help='MQTT server service port (Default: 1883)', required=False,
                    default=1883)
...
parser.add_argument('-ma', dest='mqtt_ha', action='store_true',
                    help='Switch to Home Assistant MQTT output (Default: EmonCMS)', required=False)
...
def GetRadonValue():
    ...
    # Raise exception (will try to get Radon value from RadonEye again) if received a very
    ...
    if args.silent:
        ...
        time.strftime("%Y-%m-%d [%H:%M:%S]"), mRdDeviceAddress, RadonValue, Unit))

    current_value_to_be_written = {
        "date": datetime.datetime.now().astimezone().isoformat(),
        "value": RadonValue
    }
    with open('data.json', 'a') as json_file:
        json.dump(current_value_to_be_written, json_file)
        json_file.write('\n')
    ...
    if args.mqtt:
        if args.verbose and not args.silent:
            ...
            if args.mqtt_ha:
                ...
                mqtt_out = "EmonCMS"
            print("MQTT Server: %s | Port: %s | Username: %s | Password: %s | Output: %s" % (
            args.mqtt_srv, args.mqtt_port, args.mqtt_user, args.mqtt_pw, mqtt_out))
